...
/*int maxprofit_helper(vector<int> &prices, int index, int buy, int fee, vector<vector<int>> &dp)
{

    // base condition
    if (index >= prices.size() || fee == 0)
    {
        return 0;
    }
    ...
    if (buy)
    {
        ...
        // if we buy
        int take = -prices[index] + maxprofit_helper(prices, index + 1, 0, fee, dp); // we cannot buy next stock again until we sell it

        // if we do not buy now
        int not_take = maxprofit_helper(prices, index + 1, 1, fee, dp); // we can still buy next stock
        profit = max(take, not_take);
    }
    else
    { // if we have bought, now, we are allowed to sell

        // just increament index by 2 , after completing transaction
        int sell = prices[index] - fee + maxprofit_helper(prices, index + 1, 1, fee, dp); // as we have sell, so we can buy next stock
        int not_sell = maxprofit_helper(prices, index + 1, 0, fee, dp);                   // as we have not sell, so, we can not buy next stock
...
int maxProfit(vector<int> &prices, int fee)
{
    ...
    return maxprofit_helper(prices, index, buy, fee, dp);*/
      ...
      for (int index = n - 1; index >= 0; index--)
      {
          ...
          for (int buy = 0; buy <= 1; buy++)
          {
              ...
              { // if we have bought, now, we are allowed to sell
                  ...
                  int sell = prices[index]-fee + dp[index + 1][0]; // as we have sell, so we can buy next stock
      ...
      for (int index = n - 1; index >= 0; index--)
      {
          ...
          for (int buy = 0; buy <= 1; buy++)
          {
              ...
              { // if we have bought, now, we are allowed to sell
                  ...
                  int sell = prices[index]-fee + next[0]; // as we have sell, so we can buy next stock
...
int main()
{
    ...
    int max_profit = maxProfit(prices, fee);
