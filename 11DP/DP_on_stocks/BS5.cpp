...
/*int maxprofit_helper(vector<int> &prices, int index, int buy, vector<vector<int>> &dp)
{
    ...
    { // if we have bought, now, we are allowed to sell
        ...
        int sell = prices[index] + maxprofit_helper(prices, index + 2, 1, dp); // as we have sell, so we can buy next stock
...
*/
   ...
   /* int n = prices.size();
      ...
      for (int index = n - 1; index >= 0; index--)
      {
          ...
          for (int buy = 0; buy <= 1; buy++)
          {
              ...
              { // if we have bought, now, we are allowed to sell
                  ...
                  int sell = prices[index] + dp[index + 2][0]; // as we have sell, so we can buy next stock
    ...
    // space optimization
    int n = prices.size();
    vector<int> next(2, 0),nextcurr(2,0), curr(2, 0);
    ...
    for (int index = n - 1; index >= 0; index--)
    {
        ...
        for (int buy = 0; buy <= 1; buy++)
        {
            ...
            { // if we have bought, now, we are allowed to sell
                ...
                int sell = prices[index] + nextcurr[0]; // as we have sell, so we can buy next stock
